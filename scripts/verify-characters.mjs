...
/* ---- 2. invariants across every class at every level ------------------------------------------ */
let builds = 0;
for (const id of Object.keys(db.classes)) {
  ...
  for (let level = 1; level <= 20; level++) {
    let ch;
    try {
      ch = buildCharacter(mk(id, level), db);
    } catch (e) {
      note(id, `throws at level ${level}: ${e.message}`);
      break;
    }
    builds++;
    ...
    if (got !== expect) note(id, `HP at level ${level} is ${got}, expected ${expect} (${anc} ancestry + (${db.classes[id].hpPerLevel}+${con}) x ${level})`);

    // Proficiency NEVER goes backwards as you level.
    if (prev) {
      for (const [k, v] of Object.entries(ch.proficiencies.saves))
        if (rankOf(v) < rankOf(prev.saves[k])) note(id, `${k} DROPS from ${prev.saves[k]} to ${v} at level ${level}`);
      if (rankOf(ch.proficiencies.perception) < rankOf(prev.perception))
        note(id, `perception DROPS from ${prev.perception} to ${ch.proficiencies.perception} at level ${level}`);
      for (const [k, v] of Object.entries(ch.proficiencies.attacks))
        if (rankOf(v) < rankOf(prev.attacks[k] ?? 'untrained')) note(id, `${k} attack DROPS at level ${level}`);
    }
    prev = { saves: { ...ch.proficiencies.saves }, perception: ch.proficiencies.perception, attacks: { ...ch.proficiencies.attacks } };

    // A caster must be able to cast something.
    if (db.classes[id].spellcasting && level >= 1) {
      const main = ch.spellcasting.find((e) => e.type === 'prepared' || e.type === 'spontaneous');
      if (!main) note(id, `is a caster but has no spellcasting entry at level ${level}`);
    }
  }
}
