...
| `speed` (conditional) | **"a climb Speed of 20 feet; if you already have a base climb Speed, it increases by 5 feet"** — two branches off one predicate | `speeds` carrying an **`@actor.speed.*` formula**. No new field: a non-land `speeds` grant folds as `max(existing, granted)`, so the flat number is wrong in BOTH directions (a wetlander lizardfolk's swim 15 stayed 15 where the feat says 20; a cecaelia merfolk's climb 10 jumped to 20 where the feat says 15). `speedsIf` gates only on skill rank and heritage, and `speedAdjust.add` lands *after* every grant so it cannot tell the feat's own grant from another source's. ⚠ **`min(S,1)` is the conditional** — resolveFormula has no ternary, but `min(S,1)` is an INDICATOR (1 when you have that Speed, 0 when you don't), so `min(S,1)*(S-15)+20` is exactly "S+5 if S>0, else 20", and the outer `max()` fold is then a no-op because S+5 > S. Do **not** add a provenance field to `speedAdjust`; it buys nothing this arithmetic does not already do. 3 records: `summiting-dragonblood`, `aqueous-dragonblood`, `like-a-fish-in-water` |
