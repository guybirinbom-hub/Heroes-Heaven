...
| `src/sheet/PartyMembers.tsx` | optional `localMembers?: PartyMember[]` prop; optional `renderExtra?` render-prop returning `PartyCardSlots` (`header` = the turn chip + "+" button in `.party-card-tools`, `saves` = extra rows in the left `.party-defs` block, `right` = the whole `.party-col-r` column, whose presence is what puts `.has-right` on `.party-body`); optional `onMembers?` callback (every loaded list, so the tracker's party can be built from the server party — this component mounts INSIDE the tracker's party view, which needs that party to exist first) | Delete the three props + `PartyCardSlots`; use `members`; drop the `extra` slots and the `.has-right` term, which leaves the card one full-width column |
