...
describe('inert (recorded-only) choices', () => {
  ...
  it('the number of do-nothing pickers does not creep up unnoticed', () => {
    ...
    // 47 → 49 on 2026-08-04, both from the full feat audit and both the same shape: `scroll-esoterica`
    // and `talisman-esoterica` grant temporary daily items the sheet cannot add to an inventory.
    // Structurally identical to `magical-adept`, already on this list. Recording the pick beats
    // offering nothing at all.
    const inert = Object.values(c.feats).filter((f) => f.choice?.inert).length;
    expect(inert, 'more records now record a pick without applying it').toBeLessThanOrEqual(49);
  });
